...
for(operation in operations){
    ...
    for(dataFile in dataFiles){
        ...
        #tmp = tmp[!is.na(iso3)]
        tmp = tmp[iso3 == country, .(anomaly = anomaly, nRaster = nRaster, 
                                     lats = lats, lons = lons, date = date,
                                     iso3 = iso3)]
        finalData = rbind(finalData, tmp)
        rm(tmp)
    }
    fileName = paste0("Anomalies_", country, "_m", macroResolution, "_", operation, ".rData")
